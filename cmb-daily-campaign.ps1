...
try {
    ...
    $healthCheck = node "$env:USERPROFILE\.openclaw\tools\email\campaign-health-check.mjs" --telegram --min-sent 100 2>&1
    ...
    node "$env:USERPROFILE\.openclaw\tools\email\campaign-health-check.mjs" --telegram 2>&1 | Out-Null
    exit 1
}
